...
def imagePublisher(color_image,disparity):
	pub = rospy.Publisher('/stereoCamera/colorImage/compressed', CompressedImage)
	disparitypub = rospy.Publisher('/stereoCamera/disparity', Image)
	rospy.init_node('imagePublisher', anonymous=True)
	#rospy.Subscriber('targetPose', targetPose, callbackFB)
	#rate = rospy.Rate(10) # 10hz
	#while not rospy.is_shutdown():
	#image_message = CvBridge().cv2_to_imgmsg(color_image, encoding="passthrough")
	disparity_msg = CvBridge().cv2_to_imgmsg(disparity, encoding="passthrough")
	...
	#rospy.loginfo("stereoImages publishing")
	# Publish new image
	#self.image_pub.publish(msg)

	try:
		pub.publish(msg)
		disparitypub.publish(disparity_msg)
...
try:
	...
	while True:
		...
		if valid:
			...
			if mode == "stack":
				   imagePublisher(color_image,disp_vis)
