...
%     K_in(1,1:3) =   [4.28,3.89,6*15];%[6.18,6.12,6*5.94];% 0.2*[1,1,1]; %(34/24)* %Kx 
%     K_in(2,1:3) =   [3.91,3.81,6*9.82];%[7.22,6.30,6*9.16];%   9*[1,1,1]; %Kv
%     K_in(3,1:3) =   [417, 416, 28.1]; %KR %500*[1,1,1];%30*%[23.9, 24.08 197.6];
%     K_in(4,1:3) =   [18.8, 20.0, 10.0];%KOmega % 75*[1,1,1];%10*%[9.68, 9.68, 15.56];
%     kpsi        =   73*[K_in(3,3), K_in(4,3)];
%     k_ramp      =   [1.01; 0.999; 0.651];%[1.17; 1.20; 0.247];%[0.373; 0.333; 0.247]; 
...
% testing now! and OL verified to function with these IL in conduit
    ...
    K_in(3,1:3) =   [500, 500, 70]; %KR %500*[1,1,1];%30*%[23.9, 24.08 197.6];
    K_in(4,1:3) =   [45, 45, 10];%KOmega % 75*[1,1,1];%10*%[9.68, 9.68, 15.56];
    kpsi        =   73*[K_in(3,3), K_in(4,3)];
    k_ramp      =   [1.04; 1.04; 0.651];%[1.17; 1.20; 0.247];%[0.373; 0.333; 0.247]; 
else
    % CF Gains
    K_in(1,1) = 4; %0.67;
...
if ii == 1
    position_raw = [-0.4; 0.3; 0.15];zeros(3,vIDs(inds));%[0;0;0];zeros(3,vIDs(inds));%
    pos0 = position_raw(:,1);
...
%     xref(1:6+6*(vID-1),1) = [0;0;2/pi*z_desired(vID)*atan2(tnow*5,1);0;0;0]; 
%     xref(1:6+6*(vID-1),1) = [0;0;z_desired(vID);0;0;0]; 
...
%     end
    ...
    %% - Estimation
    [xhat,zhat] = myObserverAll_wsc(xhat,zhat,pos,ang,dt,paramObs,Rmat0,Thrust_in,paramQuad);
...
%     end
    ...
    if linear == 0
        ...
        [udes,err_int,e_x_prev,e_x,e_x_std] = myTrackingCtrl_wsc_linear(xhat,xref,err_int,...
                            tnow,dt,paramQuad,paramControl,paramLinear,k_ramp,e_x_prev);
    end
    ...
    for vID = 1:Np
        ...
        elseif linear
            [u_stick{vID},psi_int{vID},angdes{vID},thrustdes(vID),psi_err_old,psi_dot_err_old] =...
                ...
                paramQuad,paramLinear,psi_err_old,dt,psi_dot_err_old);
        else
            [u_stick{vID},psi_int{vID},angdes{vID},thrustdes(vID)] =...
...
%}
    ...
    tic
%     u_stick{1}(1) = 0.1661 + u_stick{1}(1);
%     Thrust_stick = u_stick{1}(1);
% u_stick{1}(1) = 0;%0.42;
% u_stick{1}(4) = 1;%0.42;
    for vID= mod(ii,Np)+1
...
plot3(R(:,1),R(:,2),R(:,3),'.', 'linewidth', 2);hold on
...
figure(5+extra), clf
plot(time, stick_input(:,2), 'linewidth', 2), hold on
plot(time, stick_input(:,3), 'linewidth', 2)
plot(time, stick_input(:,4), 'linewidth', 2)
plot(time, stick_input(:,1), 'linewidth', 2), hold on
legend('roll', 'pitch', 'yaw', 'thrust')
title('Stick Inputs, roughly corresponding to rpyt at ICs'), grid on

figure(6+extra), clf
plot(time, nu_input(:,1), 'linewidth', 2), hold on
plot(time, nu_input(:,2), 'linewidth', 2)
plot(time, nu_input(:,3), 'linewidth', 2)
legend('roll', 'pitch', 'yaw')
title('Attitude Inputs'), grid on

% figure(7), clf
% plot(time, attitude_filter(:,4:6), 'linewidth', 2)
% grid on
% legend('p', 'q', 'r')
% title('measured rates')

% figure(9), clf
% plot(time, attitude_filter(:,1:3), 'linewidth', 2)
% grid on
% legend('roll', 'pitch', 'yaw')
% title('estimated angles')

figure(3+extra), clf
plot(time, attitude_mocap(:,1:3), 'linewidth', 2)
grid on
legend('roll', 'pitch', 'yaw')
...
for ii= 1:Np
    ...
    plot(time,R(:,1:3), 'linewidth', 2);hold on
...
figure(8+extra), clf
plot(time, attitude_desired(:,1:3), 'linewidth', 2)
grid on
legend('roll', 'pitch', 'yaw')
title('desired angles')

figure(9+extra), clf
plot(time,e_x_plot, 'linewidth', 2);hold on
plot(time,e_x_std_plot, 'linewidth',2)
legend('e_x','e_y','e_z', 'e_{xstd}','e_{ystd}','e_{zstd}')
grid on
