...
paramQuad = v2struct(mass,m_b,R,c,Nb,e,k_beta,nu_beta,Cla,Lock,th0,...
    th_tw,Omega,lam,ell,d,cm,I1,I2,I3,I,Cd_frame,rho,g,u_max,T_scale,psi_scale);


paramCF.prev_rate_error = [0;0;0];
paramCF.I_term_prev = [0;0;0];
...
paramLinear = v2struct(L_Delta_v,L_th1c,L_v,M_Delta_u,M_th1s,M_u,N_r,N_tht,...
                       X_Delta_u,X_u,Y_Delta_v,Y_v,Z_th0,Z_w);
...
A= [zeros(3), eye(3);
    zeros(3), zeros(3)];
C= [eye(3), zeros(3)];
B= [zeros(3); eye(3)];%[0,0,1;0,1,0;1,0,0]];
D= 1e2*eye(3);                   % intensity of process noise (disturbance)
