function [position, angle, t, Rmat, Quat, IDs] = get_PosAng_ID(client)
...
for ii = 1:n
    ...
    % the desk) X is toward the blinds and Z is towards the desk. 
    %
    % Our inertial frame     mocap frame
    %     [ex,ey,ez]     =  [mx,-mz,my]
    %
    % i.e, we use -z in mocap for Y in world frame (towards the back screen), 
    %         and  y in mocap for Z in world frame (towards the ceiling).
    x(ii)  =   frame.RigidBodies(ii).x;
    y(ii)  = - frame.RigidBodies(ii).z; % negative sign
