...
% desired height
z_desired = 1.45;%[0, .2, .35 , .2, 0] + 0.8; % 1.6; for chirp
xy_desired = [1; -0.5];
...
if SE3
    ...
    % rejection
    K_in(1) = 4; %0.67; %Kx
    K_in(2) = 2;%K_in(1)/1.5;%0.67; %Kv
    K_in(3) = 1500;%150; %Kr
    K_in(4) = K_in(3)/5;%/8 %Komg
else
    %for cf
    K_in(1) = 4; %0.67; %Kx
    K_in(2) = 2;%K_in(1)/1.5;%0.67; %Kv
...
psi_des = 0;% asin(cross([1;0;0],Eb1d))
...
ii= 1;
tnow= 0;
% OL_dt = 0.2/3; % Artificially prescribed time to run the outer loop
abort= 0;
...
%     end
    ...
    %% - Estimation
    [xhat,zhat] = myObserverAll_wsc(xhat,zhat,pos,ang,dt,paramObs,Rmat{1,1},Thrust_in,paramQuad);
...
%     end
    ...
    for vID = 1:Np
        ...
        % Different inputs based on yaw control method (CF is rate only)
        if SE3
...
%             u_stick{vID}(4)= 0;
        elseif tnow > tend && tnow < tend+2.7
            ...
            u_stick{vID}(1)= (1.05*delT_trim{vID}*2 - 1);%0.95*(delT_trim{vID});%%
...
%         strCmd2 = sprintf('a%.f%.f%.f%.fz',ch1cmd2,ch2cmd2,ch3cmd2,ch4cmd2);
        ...
        fprintf(sTrainerBox{vID},strCmd1);
%         fprintf('stick inputs = %2.3f, %2.3f, %2.3f, %2.3f\n',...
%             u_stick{vID}(1), u_stick{vID}(2), u_stick{vID}(3),u_stick{vID}(4))
...
%% Save workspace
%{k
folder = 'data/181231_ChirpTesting';
header = 'testIN_';
