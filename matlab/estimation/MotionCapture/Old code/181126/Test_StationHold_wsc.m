...
%% Initialization
Np    = 5; % number of guardians
Nt    = 0; % number of intruders
vIDs  = [1,2,3,4,6]; % vehicle ID specified in Motive (=ID on propeller)
inds  = 1:Np;      % vehicle index in this simulation

% desired height
z_desired = [0, .2, .35 , .2, 0] + 0.8;
...
psideslist = [pi/2, pi/2, pi/2, pi/2, pi/2];
for ii=1:Np
...
while tnow < tend+3
    ...
    % Tracking reference
    [udes,err_int] = myTrackingAccel_wsc(xhat,xref,err_int,tnow,dt,paramTracking);
    ...
    for vID = 1:Np
        ...
        [u_stick{vID},psi_int{vID},angdes{vID},thrustdes(vID)] =...
            accel2stick2(udes_i,zhat_i,psi_int{vID},dt,paramStick{vID});
    ...
    tic
    for vID= mod(ii,Np)+1
        ch1cmd = 5000+ 4000*u_stick{vID}(1);  % throttle (up)
        ch2cmd = 5000- 4000*u_stick{vID}(2);  % roll     (right)
        ch3cmd = 5000+ 4000*u_stick{vID}(3);  % pitch    (forward)
        ch4cmd = 5000+ 4000*u_stick{vID}(4);  % yaw
...
plot3(rref(1)+Rmax*cos(angc),rref(2)+Rmax*sin(angc),ones(size(angc))*rref(3),'--','color',[.5,.5,.5],'linewidth',1)
grid on
xlabel('x')
ylabel('y')
