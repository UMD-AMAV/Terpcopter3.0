...
throttle_PWM = floor(((throttle_trainer - 4800)/6) + 1500);
roll_PWM = floor(((roll_trainer - 4800)/6) + 1500);
pitch_PWM = floor(((pitch_trainer - 4800)/6) + 1500);
yaw_PWM = floor(((yaw_trainer - 4800)/6) + 1500);
...
dt = 0.001; % timestep for solver, plot % 0.08 seems to match real time
...
for ii = 1:(N-1)
    [time,z_indiv] = ode45(@eom, tspan(ii:ii+1), z0, options,...
                                    R_d_old,dt,Eb3_in,K_in,u_stick,disturbance,Euler_old);
    ...
    [zdot(:,ii+1), M_B(:,ii+1), OMGA(:,ii+1), input_T(:,ii+1), R_d(:,:,ii+1), R_B(:,:,ii+1), Euler(:,ii)]...
        = eom(time(end),z0,R_d_old,dt,Eb3_in,K_in,u_stick,disturbance,Euler_old);
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [zdot, M_B, OMGA, T, R_d, R_B, Euler] = eom(t, z,R_d_old,dt,Eb3_in,...
                                        K_in,u_stick,disturbance,Euler_old)
...
Euler = [0;0;0];
%{
thetapid_d = -asin(R_d(3,1));
phipid_d = atan2(R_d(3,2)/cos(thetapid_d),R_d(3,3)/cos(thetapid_d));
psipid_d = atan2(R_d(2,1)/cos(thetapid_d),R_d(1,1)/cos(thetapid_d));
% 
...
p_d = Euler_dot(1) - sin(thetapid)*Euler_dot(3);
q_d = cos(phipid)*Euler_dot(2) + sin(phipid)*cos(thetapid)*Euler_dot(3);
r_d = -sin(phipid)*Euler_dot(2) + cos(phipid)*cos(thetapid)*Euler_dot(3);
e_omg = OMGA - [p_d; q_d; r_d];

% disp([thetapid_d thetapid; phipid_d phipid; psipid_d psipid])   
% e_omg = 
